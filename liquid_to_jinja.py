...
def safe_re_sub(pattern, repl, string, *args, **kwargs):
    """Safe regex substitution that handles None values"""
    if string is None:
        return ''
...
def convert_liquid_to_jinja(liquid_template):
    ...
    if liquid_template is None:
        return ''
    
    # Convert to string if not already a string
    if not isinstance(liquid_template, str):
        liquid_template = str(liquid_template)
    
    # Handle empty strings
    if not liquid_template.strip():
        return liquid_template
    
    # Convert comments
    jinja_template = safe_re_sub(r'{%-?\s*comment\s*-?%}(.+?){%-?\s*endcomment\s*-?%}', r'{# \1 #}', liquid_template, flags=re.DOTALL)

    jinja_template = safe_re_sub(r'{{\s*(\w+)\s*\|\s*truncate:\s*(\d+)\s*}}',  r'{{ \1[:\2] }}', jinja_template)

    jinja_template = safe_re_sub(
    ...
    # Convert default attribute filters
    jinja_template = re.sub(r'\{\{\s*\$\{first_name\}\s*\}\}', r"{{UserAttribute['First Name']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{last_name\}\s*\}\}', r"{{UserAttribute['Last Name']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{email_address\}\s*\}\}', r"{{UserAttribute['Email (Standard)']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{language\}\s*\}\}', r"{{UserAttribute['language']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{country\}\s*\}\}', r"{{UserAttribute['country']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{user_id\}\s*\}\}', r"{{UserAttribute['UserID']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{most_recent_location\}\s*\}\}', r"{{UserAttribute['last known city']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{date_of_birth\}\s*\}\}', r"{{UserAttribute['Birthday']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{gender\}\s*\}\}', r"{{UserAttribute['Gender']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{city\}\s*\}\}', r"{{UserAttribute['Last Known City']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{last_used_app_date\}\s*\}\}', r"{{UserAttribute['Last Seen']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{most_recent_app_version\}\s*\}\}', r"{{UserAttribute['OS Version iOS']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{time_zone\}\s*\}\}', r"{{UserAttribute['User Time Zone Offset (Mins)']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{most_recent_locale\}\s*\}\}', r"{{UserAttribute['Locale Language']}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*\$\{braze_id\}\s*\}\}', r"{{UserAttribute['ID']}}", jinja_template)


    jinja_template = re.sub(r'\{\{\s*default\s*:\s*(\w+)\s*\}\}', r"{{UserAttribute.get('\1', '')}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*default\s*:\s*(\w+)\s*\}\}', r"{{CampaignAttribute.get('\1', '')}}", jinja_template)
    jinja_template = re.sub(r'\{\{\s*default\s*:\s*(\w+)\s*\}\}', r"{{ContentBlock.get('\1', '')}}", jinja_template)
